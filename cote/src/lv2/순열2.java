...
public class 순열2 {
    ...
    public static List<List<Integer>> generatePermutations(List<Integer> elements) {
        ...
        for (int i = 0; i <= elements.size(); i++) {
            backtrack(result, new ArrayList<>(), elements, i);
        }
        return result;
    }

    private static void backtrack(List<List<Integer>> result, List<Integer> tempList, List<Integer> elements, int k) {
        if (tempList.size() == k) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            backtrack(result, tempList, elements, k);
